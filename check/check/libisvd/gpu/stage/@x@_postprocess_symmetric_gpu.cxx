...
static void test( char dista, char ordera, const JobUV jobuv ) {
  ...
  switch ( jobuv ) {
    ...
    case BlockUV: {
      ...
      // Run stage
      isvd_@x@PostprocessSymmetric_gpu(param, nullptr, 0, nullptr, 0, dista_, ordera_, a, lda, qt, ldqt, s, ut, ldut, -1);
    ...
    case NoUV: {
      ...
      // Run stage
      isvd_@x@PostprocessSymmetric_gpu(param, nullptr, 0, nullptr, 0, dista_, ordera_, a, lda, qt, ldqt, s, nullptr, 0, -2);
...
TEST(@XStr@_SymmetricPostprocessing_Gpu, BlockCol_RowMajor_GatherUV) {
  test('C', 'R', GatherUV);
}

TEST(@XStr@_SymmetricPostprocessing_Gpu, BlockRow_ColMajor_GatherUV) {
  test('R', 'C', GatherUV);
}

TEST(@XStr@_SymmetricPostprocessing_Gpu, BlockRow_RowMajor_GatherUV) {
  test('R', 'R', GatherUV);
}

TEST(@XStr@_SymmetricPostprocessing_Gpu, BlockCol_ColMajor_BlockUV) {
  test('C', 'C', BlockUV);
}

TEST(@XStr@_SymmetricPostprocessing_Gpu, BlockCol_RowMajor_BlockUV) {
  test('C', 'R', BlockUV);
}

TEST(@XStr@_SymmetricPostprocessing_Gpu, BlockRow_ColMajor_BlockUV) {
  test('R', 'C', BlockUV);
}

TEST(@XStr@_SymmetricPostprocessing_Gpu, BlockRow_RowMajor_BlockUV) {
  test('R', 'R', BlockUV);
}


TEST(@XStr@_SymmetricPostprocessing_Gpu, BlockCol_ColMajor_NoUV) {
  test('C', 'C', NoUV);
}

TEST(@XStr@_SymmetricPostprocessing_Gpu, BlockCol_RowMajor_NoUV) {
  test('C', 'R', NoUV);
}

TEST(@XStr@_SymmetricPostprocessing_Gpu, BlockRow_ColMajor_NoUV) {
  test('R', 'C', NoUV);
}

TEST(@XStr@_SymmetricPostprocessing_Gpu, BlockRow_RowMajor_NoUV) {
  test('R', 'R', NoUV);
}
